...
public class InformationOnePageServiceImpl implements InformationOnePageService {
    ...
    public int delInfoOnePage(Long infoOPId, Long userId) {
        ...
        // 通知缓存 发生变化
        redisAdapter.delete(CacheKeyConstant.INFORMATIONONEPAGE_KEY);
    ...
    public List<InformationOnePage> selectInfoOPByTempAndTag(Long tempId, Long infoOPTagId) {
		...
		List<InformationOnePage> list = (List<InformationOnePage>) redisAdapter.get(CacheKeyConstant.INFORMATIONONEPAGE_KEY);
        ...
        list = infoOnePageMapper.selectByTempTag(map);
        redisAdapter.put(CacheKeyConstant.INFORMATIONONEPAGE_KEY, new ArrayList<>(list));
        return  list;
    }
