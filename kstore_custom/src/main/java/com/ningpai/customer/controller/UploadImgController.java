...
public class UploadImgController {
    ...
    public void uploadShareImg(MultipartHttpServletRequest request, HttpServletResponse resp, Long customerId)
            throws IOException {
        ...
        // 判断当前登陆的用户是否是当前用户
        if (null == customerId) {
            msg = "what are you doing young boy heihei ...???????????????????????????????????";
            out.append("<script>parent.callback('" + msg + "');</script>");
            return;
        }

        Long cid = (Long) request.getSession().getAttribute(CustomerConstantStr.CUSTOMERID);
    ...
    @ResponseBody
    public void uploadImageBackUrl(MultipartHttpServletRequest request, HttpServletResponse response)
            throws IOException {
