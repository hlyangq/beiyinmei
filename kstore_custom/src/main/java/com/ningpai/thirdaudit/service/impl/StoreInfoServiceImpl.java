...
public class StoreInfoServiceImpl implements StoreInfoService {
    ...
    public int delStoreInfoById(String[] storeInfoIds, String username) {
        if (storeInfoIds != null && storeInfoIds.length > 0) {
            ...
            // 另起线程删除索引
            new Thread(new DeleteIndexThread(storeInfoIds)).start();
    ...
    private class DeleteIndexThread implements Runnable {
        ...
        public DeleteIndexThread(String[] storeInfoIds) {
            this.storeInfoIds = storeInfoIds;
        }

        @Override
        public void run() {
            goodsElasticSearchService.deleteGoodsByThirdId(storeInfoIds);
        }
    }
    ...
    public StoreInfo selectByCustomerId(Long customerId) {
        return storeInfoMapper.selectByCustomerId(customerId);
    }
