...
public class ApplyBrandvoController {
    ...
    public ModelAndView updateApplyBrand(Long[] applyBrandIds, String reason, String applyStatus) {
        ...
        if ("1".equals(applyStatus)) {
            Long[] appl = new Long[applyBrandIds.length];
            for (int i = 0; i < applyBrandIds.length; i++) {
                appl[i] = applyBrandIds[i];
                ApplyBrand applyBrand = storeCommonService.selectByApplyBrandId(appl[i]);
                if(applyBrand != null && applyBrand.getApplyThirdId() != null){
                    storeCommonService.applyThirdBrandToTrue(appl[i],applyBrand.getApplyThirdId());
                }
            }
        }
