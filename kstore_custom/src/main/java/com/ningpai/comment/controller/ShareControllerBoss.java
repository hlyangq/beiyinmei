...
public class ShareControllerBoss {
    ...
    public ModelAndView initShareList(PageBean pageBean, @Valid ShareVo shareVo, String[] attr, String create, String createT) {
        ...
        try {
            ...
            resultMap.put(CustomerConstantStr.PAGEBEAN, shareServiceMapper.selectAllShareByShare(pageBean, shareVo,0L));
    ...
    public ModelAndView initShareList(HttpServletRequest request, PageBean pageBean, @Valid ShareVo shareVo, String[] attr,String n,String l) {
        ...
        try {
            resultMap.put("attr", attr);
            resultMap.put(CustomerConstantStr.SHARE, shareVo);
            resultMap.put(CustomerConstantStr.PAGEBEAN, shareServiceMapper.selectAllShareByShare(pageBean, shareVo, (Long) request.getSession().getAttribute("thirdId")));
            return new ModelAndView(CustomerConstantStr.THIRD_SHARELIST).addAllObjects(resultMap).addObject("storeName",(String)request.getSession().getAttribute("storeName"));
        } finally {
            resultMap = null;
        }
    }
    ...
    public ModelAndView deleteShare(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        try {
            ...
            if (ArrayUtils.isEmpty(shareIds)) {
