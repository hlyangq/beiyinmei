...
<script>
    ...
    <style>
        .export-btn .btn {margin:0!important;}
    </style>
</head>
                        ...
                        <form action="queryThirdOrderList.html" class="searchThirdOrderList" method="post">
                            ...
                            <div class="filter-groups">
                                ...
                                <div class="control-group lg-group">
                                    ...
                                    <div class="controls">
                                        <input type="text" name="startTime" id="startTime" value="<#if map.order.startTime??>${map.order.startTime}</#if>" class="form-control sm-input datepicker" data-provide="datepicker"/>
                                        ~
                                        <input type="text" name="endTime" id="endTime" value="<#if map.order.endTime??>${map.order.endTime}</#if>" class="form-control sm-input datepicker" data-provide="datepicker"/>
                                    </div>
                                </div>
                                <div class="control-group">
                                    <label class="control-label">收货人：</label>
                                    <div class="controls">
                                        <input class="form-control" type="text" name="shippingPerson" id="shippingPerson" value="<#if map.order.shippingPerson??>${map.order.shippingPerson}</#if>"/>
                                    </div>
                                </div>
                                <div class="control-group lg-group">
                                    <label class="control-label">订单价格：</label>
                                    <div class="controls">
                                        <input  class="form-control sm-input" name="startprice" id="startprice"
                                                onkeyup="value=value.replace(/[^\d]/g,'') "
                                                onbeforepaste="clipboardData.setData('text',
								            clipboardData.getData('text').replace(/[^\d]/g,''))"
                                        ...
                                        ~
                                        <input  class="form-control sm-input" name="endprice" id="endprice"
                                                onkeyup="value=value.replace(/[^\d]/g,'') "
                                                onbeforepaste="clipboardData.setData('text',
								            clipboardData.getData('text').replace(/[^\d]/g,''))"
                            ...
                            <div class="search-operation">
                                ...
                                <div class="btn-group export-btn">
                                    <a type="button" class="btn btn-sm" href="javascript:;" onclick="exportAllOrder()">导出所有订单</a>
                                    <a type="button"
                                       class="btn btn-sm dropdown-toggle" data-toggle="dropdown" aria-expanded="false" onclick="menu_btn(this)">
                                        <span class="caret"></span>
                                    </a>
                                    <ul class="dropdown-menu" role="menu" style="min-width: 135px;font-size:12px;padding:2px 0;">
                                        <li>
                                            <a href="javascript:;" onclick="exportCheckOrder()">导出选中</a>
                                        </li>
                                    </ul>
                                </div>
                            </div>
                                    ...
                                    <#list map.pb.list as order>
                                        ...
                                        <tr class="header-row">
                                            <td>
                                                ...
                                                <span style="margin-left:20px;">订单号：${order.orderCode}</span>
                                            </td>
                                            <td class="row-ops" colspan="6">
                                        ...
                                        <tr class="content-row">
                                            <td class="order-pros">
                                                ...
                                                <#if order.orderGoodsList??>
                                                 <#list order.orderGoodsList as maps>
                                                     <#if maps.goodsProductVo??>
                                                         <a target="_blank" title="${maps.goodsProductVo.goodsInfoName!'' }" href="${map.bset!''}/item/${maps.goodsProductVo.goodsInfoId!''}.html"><img src="${maps.goodsProductVo.goodsInfoImgId!'' }" alt="商品1" width="60" height="60"/></a>
                                                     </#if>
                                                 </#list>
                                                </#if>
                                            </td>
                                            ...
                                            <td>
                                                <div class="btn-group">
                                                     <#if order.orderStatus=="0"&&order.orderLinePay=="1">
                                                         <button type="button" class="btn btn-primary btn-sm" data-toggle="modal" data-target="#modifyOrder" onclick="modifyThirdOrderbyId(<#if order.orderId??>${order.orderId}</#if>,<#if order.expressPrice??>${order.expressPrice}</#if>)">修改金额</button>
                                                     ...
                                                     <#if map.isPage =="0">
                                                        <#if order.orderCargoStatus == '0'>
                                                            <button type="button" class="btn btn-primary btn-sm" style="width: 86px;" onclick="javascript:window.location.href='querythirdoutstock.html?orderCargoStatus=0'">去拣货</button>
                                                        <#elseif order.orderCargoStatus == '1'>
                                                            <button type="button" class="btn btn-primary btn-sm" style="width: 112px;" onclick="javascript:window.location.href='querythirdoutstock.html?orderCargoStatus=1'">去装箱</button>
                                                        <#elseif order.orderCargoStatus == '2'>
                                                            <button type="button" class="btn btn-primary btn-sm" style="width: 112px;" onclick="javascript:window.location.href='querythirdoutstock.html?orderCargoStatus=2'">去出库</button>
                                                        </#if>
                                                        <#if order.orderCargoStatus == '0'>
                                                            <button type="button" class="btn btn-primary btn-sm dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                                                                <span class="caret"></span>
                                                            ...
                                                            <ul class="dropdown-menu" role="menu">
                                                                <li><a href="" data-toggle="modal" onclick="showThirdOneKeyDelivery(${order.orderId},${order.orderMType});">一键发货</a></li>
                                                            </ul>
                                                        </#if>
                                                     <#else >
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form class="update_thirdorder" action="updateThirdOrder.htm" method="post">
                    ...
                    <input type="hidden" name="express_Price" class="express_Price" value="" />
                    <div class="form-item">
                        <label class="control-label">订单编号：</label>
...
<div class="modal fade" id="thirdOneKeyDelivery" role="dialog" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
            ...
            <div class="modal-body">
                <form class="delivery_thirdorder" action="updateThirdOrder.htm" method="post" id="thirdOneKeyDeliveryForm">
                    <input type="hidden" name="orderId" id="deliveryId" value="" />
                    <div class="form-item">
                        <label class="control-label">物流公司：</label>
                        <input type="hidden" name="expressId" value="" id="expressId" />
                        <input type="hidden" id="orderMType" />
                        <div class="controls checkWp upt_orderCode" id="expressName"></div>
                    </div>
                    <div class="form-item">
                        <label class="control-label">物流单号：</label>
                        <div class="controls">
                            <input type="hidden" name="thirdId" value="${map.order.businessId}">
                            <input type="text" minlength="6" maxlength="20" id="expressNo" value="" name="expressNo" class="form-control"/>
                            <label class="upt_expressInfo"></label>
                        </div>
                    </div>
                </form>
            </div>
            <div class="modal-footer">
                <button class="btn btn-default" type="button" data-dismiss="modal">关闭</button>
                <button class="btn btn-primary" onclick="thirdOneKeyDelivery()" type="button">确定</button>
            </div>
        </div>
    </div>
</div>
<form  method="post" action="exportallorder.htm" id="ordercxform">
    <input type="hidden" name="orderCode" id="orderCodestr" value="" />
    <input type="hidden" name="startTime" value="" id="startTimestr" />
    <input type="hidden" name="endTime" value="" id="endTimestr" />
    <input type="hidden" name="shippingPerson" value="" id="shippingPersonstr" />
    <input type="hidden" name="startprice" value="" id="startpricestr" />
    <input type="hidden" name="endprice" value="" id="endpricestr" />
    <input type="hidden" name="orderStatus" value="" id="orderStatusstr" />
</form>

<form action="printView.htm?" id="printView" method="post" target="_blank">
...
<script>
    ...
    function exportAllOrder(){
        ...
        $("#orderStatusstr").val($(".tabStatus").val());
        $("#startpricestr").val($("#startprice").val());
        $("#endpricestr").val($("#endprice").val());
        $("#ordercxform").submit();
    }
    ...
    function exportCheckOrder(){
        if (checkSelected("orderId")) {
            $("#printView").attr("action","exportcheckedorder.htm?type=2&orderIds="+checkedList);
            $("#printView").submit();
            $("#printView").attr("action","printView.htm");
        }
        else {
