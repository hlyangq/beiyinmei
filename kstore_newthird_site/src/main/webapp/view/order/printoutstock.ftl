...
<body>
	...
	<div class="print_wp " >
        ...
        <div class="modal-body">
           ...
           <#list relations as relation>
            <h4 class="picking-box-tit">装箱单${relation_index+1 }</h4>
               ...
               <table class="table table-bordered">
                   <thead>
                   <tr>
                       <th width="148px">商品编号</th>
                       <th width="126px">商品名称</th>
                       <th width="124px">商品规格</th>
                       <th width="48px">价格</th>
                       <th width="79px">商品数量</th>
                       <th width="49px">小计</th>
                       <th class="Noprint" width="95px">操作</th>
                   </tr>
                   </thead>
                   <#list relation.containers as containers>
                       <tbody>
                           ...
                           <tr>
                               ...
                               <td>
                                   <#--<#if containers.goodsProductDetailViewVo.goodsInfoName?length gt 30>-->
                                   <#--${containers.goodsProductDetailViewVo.goodsInfoName?substring(0,30)}...-->
                                   <#--<#else>-->
                                   ${containers.goodsProductDetailViewVo.goodsInfoName}
                                   <#--</#if>-->
                               </td>
                               <td>
                                   <#list containers.goodsProductDetailViewVo.specVo as specVo>
                                   ${specVo.spec.specName}:

                                       <#if specVo.specValueRemark??>${specVo.specValueRemark}<#else>${specVo.spec.specRemark}</#if><br/>
                                   </#list>
                               </td>
                               ...
                               <td class="Noprint">
                                   <button class="btn btn-primary btn-sm" onclick="changeContainers(${containers.containerId},${containers.goodsNum},${containers.relationId})" type="button" data-toggle="modal" data-target="#changeBox">更换包裹</button>
           ...
           <button class="btn btn-primary Noprint" onclick="delContainers(${relation.relationId})" type="button">删除此装箱单</button>
           </#list>
            <!--endprint1-->
        </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form  action="thirdupdatecontainer.htm" class="changeContainer" method="post">
                        ...
                        <input  type="hidden" name="curRelationId" id="curRelationId" />
                        <div class="modal-body">
                            请选择装箱单：
    ...
    <div class="modal fade" id="ZXout_success" role="dialog" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" onclick="$('#ZXout_success').modal('hide');"  data-dismiss="modal">确定</button>
                </div>
            </div>
        </div>
    </div>
