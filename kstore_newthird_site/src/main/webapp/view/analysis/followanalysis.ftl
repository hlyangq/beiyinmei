            ...
            <div class="app-content">
                <div class="chart-operation">
                    <form action="followanalysis.htm" method="post" id="downform">
                        ...
                        <input type="hidden" name="oneCategoryValue" value="${oneCategoryValue!''}"/>
                        <input type="hidden" name="twoCategoryValue" value="${twoCategoryValue!''}"/>
                        <input type="hidden" name="threeCategoryValue" value="${threeCategoryValue!''}"/>
                        选择分类：
                        <select class="form-control sm-sel" id="cat_first" name="oneSelectCategoryValue" onChange="loadSecondCat(this.value);"></select>
                        <select class="form-control sm-sel" id="cat_second" name="twoSelectCategoryValue" onChange="loadThirdCat(this.value);"></select>
                        <select class="form-control sm-sel" id="cat_third" name="catId"></select>
                        <select class="form-control sm-sel" id="search_by_time" name="selectIndex" onChange="searchbytime()">
                            <option value="0" <#if selectindex==0>selected="selected" </#if> >-请选择-</option>
                            <option value="1" <#if selectindex==1>selected="selected" </#if> >本周</option>
                            <option value="2" <#if selectindex==2>selected="selected" </#if> >上周</option>
                            <option value="3" <#if selectindex==3>selected="selected" </#if> >本月</option>
                            <option value="4" <#if selectindex==4>selected="selected" </#if> >上月</option>
                        </select>
                        <input class="form-control sm-input datepicker" value="${startTime!''}" name="startTime" id="d4311" data-provide="datepicker"
                               onchange="checkstarttime(this)"
                               />
                        -
                        <input class="form-control sm-input datepicker" value="${endTime!''}" name="endTime" id="d4312"  data-provide="datepicker"
                               onchange="checkendtime(this)"
                                />
                ...
                <div class="data-chart-cont" id="chartCont" style="width:880px; margin-left: -45px; height: 330px; cursor: default; background-color: rgba(0, 0, 0, 0);" _echarts_instance_="1418354017304">
                </div>
...
<script type="text/javascript">
    ...
    function checkstarttime(obj){
        ...
        var arrs = $("input[name='endTime']").val().split("-");
        var lktime = new Date(arrs[0], arrs[1], arrs[2]);
        var lktimes = lktime.getTime();

        if (starttimes > lktimes) {
            $(obj).val($("input[name='endTime']").val());
        }
    }
    ...
    function checkendtime(obj){
        ...
        var arrs = $("input[name='startTime']").val().split("-");
        var starttime = new Date(arrs[0], arrs[1], arrs[2]);
        var starttimes = starttime.getTime();

        if (starttimes > endtime) {
            $(obj).val($("input[name='startTime']").val());
        }
    }
