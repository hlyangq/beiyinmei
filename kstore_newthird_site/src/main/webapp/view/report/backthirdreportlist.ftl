                ...
                <div class="cfg-content">
                    <table class="table">
                        ...
                        <tr>
                            ...
                            <form class="simple_search" action="<#if pb.url??>${pb.url}<#else></#if>" method="post">
                            </form>
                            <form class="high_search" action="<#if pb.url??>${pb.url}<#else></#if>" method="post">
                                <#if pb.list?size gt 0>
                                    <#list pb.list as backOrder>
                                        <tbody>
                                        <tr>
                                            <td>${backOrder_index+1}</td>
                                            <td>${backOrder.backOrderCode }</td>
                                            <td>${backOrder.orderCode }</td>
                                            <td><#if backOrder.backPrice??>${backOrder.backPrice?string("0.00") }</#if></td>
                                            <td><#if backOrder.backOrderCatePrice??>${backOrder.backOrderCatePrice?string("0.00") }</#if></td>
                                            <td>${backOrder.backTime?string("yyyy-MM-dd") }</td>

                                            <td> 退款</td>
                                            <td style="width: 17%">
                                                <div class="btn-group">
                                                    <button type="button" class="btn btn-primary btn-sm" onclick="window.location.href='queryThirdReportBackOrderCateDetail.htm?orderId=${backOrder.orderId }'">查看明细</button>
                                                </div>
                                            </td>
                                        </tr>
                                        </tbody>
