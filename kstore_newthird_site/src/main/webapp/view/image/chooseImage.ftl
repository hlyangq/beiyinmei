    ...
    function doAjaxForCate(pageNo, pageSize,thirdId) {
        ...
        $.get("ajaxQueryImageForChoose.htm?classifyId=-1&pageNo=" + pageNo + (pageSize == undefined ? "" : ("&pageSize=" + pageSize))+"&thirdId="+thirdId,
                function (data) {
                    ...
                    foot = foot + "<a onclick='doAjaxForCate(" + data.nextPageNo + "," + data.pageSize + "," + thirdId + ")' href='javascript:void(0)'>&raquo;</a></div></div></div>";
                    /*添加tfoot分页信息*/
                    $("#tf").html(foot);
                    /*进行递归  GO*/
    ...
    function callback(msg) {
        ...
        if(msg.imgurl=="-1"){
            ...
            if(n>-1){
                ...
                var _li = '<li><img alt="" src="'+msg.imgurl+'" style="max-width: 230px; width: auto; height: 230px;"></li>';
                $(".imgs_list").append(_li);
                if($("#fileName").val().length>0){
