            ...
            <div class="app-content">
                <div>
                    ...
                    <div class="cfg-content">
                        <div class="ops-bar">
                            <button class="btn btn-primary btn-sm" id="save_btn-sm" type="button" onclick="addLogisticsSingle(${thirdId})"><i class="glyphicon glyphicon-plus"></i>添加</button>
                            <label class="add-tips">(物流单和物流公司一一对应)</label>
                            <label class=" add_tip_is_error" style="color:red;display: none">请先添加物流公司！</label>
                        </div>
                        <table class="table">
                            <thead>
	                            <tr>
...
<script>
    //添加物流单
    function addLogisticsSingle(storeid){
        $.ajax({
            type:'post',
            url:'${basePath}/valiLogisticsSingle.htm',
            data:"storeId="+storeid,
            success:function(data){
                if(data==0){
                    $(".add_tip_is_error").css("display","block");
                    $(".add_tip_is_error").hide(5000);
                }else{
                    window.location.href='toAddThirdLogisticsSingle.htm';
                }
            }
        })
    }
