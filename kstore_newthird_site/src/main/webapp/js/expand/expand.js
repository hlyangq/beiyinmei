$(function() {
	...
	$.get("queryAllGrandCateForThirdtwo.htm?cateId=" + 0, function(data) {
		...
		for ( var i = 0; i < data.length; i++) {
			if($("input[name='oneCategoryValue']") !=undefined &&
					$("input[name='oneCategoryValue']").val() !='' &&
					$("input[name='oneCategoryValue']").val() == data[i].catId
			){
				html += "<option selected value='"+ data[i].catId +"'>"
						+ data[i].catName
						+ "</option>";
				loadSecondCat(data[i].catId);
			}else{
...
/*选择分类*/
function loadSecondCat(value){
	$.get("queryAllGrandCateForThirdThree.htm?cateId=" + value, function(data) {
		var html = "<option value=''>-请选择-</option>";
		for ( var i = 0; i < data.length; i++) {
			if($("input[name='twoCategoryValue']") !=undefined &&
					$("input[name='twoCategoryValue']").val() !='' &&
					$("input[name='twoCategoryValue']").val() == data[i].catId
			){
				html += "<option selected value='"+ data[i].catId +"'>"
						+ data[i].catName
						+ "</option>";
				loadThirdCat(data[i].catId);
			}else{
				html += "<option value='"+ data[i].catId +"'>"
						+ data[i].catName
						+ "</option>";
...
function loadThirdCat(value){
	$.get("queryAllGrandCateForThirdThree.htm?cateId=" + value, function(data) {
		var html = "<option value=''>-请选择-</option>";
		for ( var i = 0; i < data.length; i++) {
			if($("input[name='threeCategoryValue']") !=undefined &&
					$("input[name='threeCategoryValue']").val() !='' &&
					$("input[name='threeCategoryValue']").val() == data[i].catId
			){
				html += "<option selected value='"+ data[i].catId +"'>"
						+ data[i].catName
						+ "</option>";
