...
function choosePro(){
	...
	$(".result-table .choosepro").each(function(){
		...
		html +="<td name='goodsPrices'>"+$(this).attr("data-price")+"</td>";
...
function chooseZKPro(){
	...
	$(".result-table .choosepro").each(function(){
		...
		if(dataname.length>5){
			subdataname = dataname.substr(0,5)+'...';
		}else{
			subdataname = dataname
		}

			html += "<tr><input type='hidden' name='goodsIdP' value='"+$(this).attr("data-id")+"'>";
		html +="<td>"+$(this).attr("data-no")+"</td>";
		html +="<td title="+dataname+">"+subdataname+"</td>";
		...
		html +="<td><button class='btn btn-primary btn-sm' onclick='mofensingle("+$(this).attr("data-price")+",this,1);' type='button'>移除分</button>";
...
function mofensingle(price,obj,fixed){
    ...
    var tmp  = dianQ2.length;
    var b = 0;
    if(fixed==1){
        b = dianQ2.substring(0,tmp-1);
        $(obj).parents("tr").find("input[name='discountFlag']").val(1);
    }else if(fixed==0){
        b = dianQ2.substring(0,tmp-3);
