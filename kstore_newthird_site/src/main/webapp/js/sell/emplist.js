    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            //<label class="choose-label"><input name="role" type="radio"/>新职位</label>
                ...
                str+='<label class="choose-label" onclick="clearEmployeeTip()"><input type="radio" value="'+data.roles[i].id+'" name="authId" />'+data.roles[i].designation+'</label>';
            }
...
function clearEmployeeTip(){
    $(".clearEmployeeTip").html("");
}
...
function validateForm(){
    ...
    if(name==null||$.trim(name)==""){
        ...
        $("#customerUsername").next().html("用户名长度至少为4位，且不能包含特殊字符及其中文");
        flag&=false;
    }else{
        $.ajax({
            type: 'post',
