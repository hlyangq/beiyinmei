...
function modifyemptodisable(custId,flag){
	...
	//判断默认条数
	if(flag == 0 ){
		$("#update_flag").find(".control-label").html("你确定要开启吗?");
	}else {
        $("#update_flag").find(".control-label").html("你确定要关闭吗?");
	}
		...
		$.ajax({
			...
			success: function(data) {
				if(data>0){
                    ...
                    //物流名称是当前名称的话，就不做判断物流公司是否已经存在
                    if($("#oldExpName").val() == $(".expname_updatexpress").val()){
                        $("#updatebutton").removeAttr("onclick");
                        $("#expform_updates").submit();
                    }else{
                        $.ajax({
                            type: 'post',
                            url:"checkExpressName.html?expressName="+$(".expname_updatexpress").val(),
                            async:true,
                            success: function(data) {
                                if(data>0){
                                    $(".expname_Tip").html("物流名称不能重复！");
                                    $(".expname_Tip").css("color","red");
                                }else{
                                    $("#updatebutton").removeAttr("onclick");
                                    $("#expform_updates").submit();
                                }
		...
		$.ajax({
			...
			success: function(data) {
				if(data>0){
                    ...
                    $.ajax({
                        type: 'post',
                        url:"checkExpressName.html?expressName="+$(".expname_addexpress").val(),
                        async:true,
                        success: function(data) {
                            if(data>0){
                                $(".expname_Tip").html("物流名称不能重复！");
