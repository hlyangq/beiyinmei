...
$(function(){
    ...
    function checktime(){
        ...
        if(companyCreTime==''){
            ...
            var birth = new Date(Date.parse(companyCreTime + " 00:00:00"));
            var nowDate = new Date();
            if (birth.getTime() > nowDate.getTime()) {
                $('#companyCreTimeTip').html("成立日期不能大于当前日期");
                $('#companyCreTimeTip').css("color", "red");
                flag = false;
            } else {
        ...
        if (bussDate == '') {
            ...
            var birth = new Date(Date.parse(companyCreTime + " 00:00:00"));
            var startTime = new Date(Date.parse(bussDate + " 00:00:00"));
            if (birth.getTime() != startTime.getTime()) {
                $('#bussDateTip').html("有效期的起始日期必须和成立日期一致");
                $('#bussDateTip').css("color", "red");
                flag = false;
                return flag;
            } else {
    ...
    function check(){
       ...
       if(!re.test(bussTaxCredUrlE) && bussTaxCredUrlE!=""){
        ...
        //    flag = false;
        //}
        if(!validateChkFun.formSubmit(['#bussId','#bussRange','#bussLegalName','#bussLegalCardId','#bussAddr'])){
            flag = false
        }
        if(flag){
    ...
    $(".want_btn").click(function() {
    	var flag= validateChkFun.formSubmit([ '#storeName','#cids','#bdids']);
    	if($(".hasa").val().length != "0" && $(".hasbd").val().length != "0" ){
    	}else{
    	...
    	if(flag){
            ...
            $.ajax({
                url:"checkStoreNameIsUsing.htm?storeName="+storeName,
                type:"post",
                async:false,
	...
	$("#customer_service_btn").click(function(){
		var flag= true;
        var qqSwitch = $("input[name='qq']:checked").val() ;
        $("#qqSwitch").val(qqSwitch);
        if(!isNaN($("input[name='serviceQq']").val())){
            if(flag){
    ...
    $("#serviceQq").focus(function(){
        $("#serviceQq").removeClass("error");
        $("#serviceQqTip").text("");
        $("#serviceQq").parent("div").find("span").hide();
    });
    ...
    validateChkFun.init(function() {
        ...
        validateChkFun.defaultChk({
          ...
          validateChkFun.defaultChk({
        	...
        	onError: {
        		badFormat: "公司电话格式不正确(格式：xxx-xxxxxxxx)"
        	}
        });
