...
//更新
function modifyThirdOrderbyId(id,expressPrice){
	$(".searchOrderList").hide();
    $(".express_Price").val(expressPrice);
...
function uptThirdOrder(){
        var expressPrice = $(".express_Price").val();
        var orderPrice = $("#orderPrice").val();
	    var isT=true;
        var fairly = $("#orderPrice").val() - $(".express_Price").val();
        isT = checkRegexp( $("#orderPrice"),/^\d+(\.\d{1,2})?$/, "价格必须是正整数或两位小数点" , $(".upt_orderInfo")) && isT;
        if (fairly > 0) {
            $("#orderPrice").removeClass( "ui-state-error" );
            $(".upt_orderInfo") .text( "" ) .addClass( "ui-state-highlight" );
            isT = true;
        }
        else {
            $("#orderPrice").addClass( "ui-state-error" );
            $(".upt_orderInfo") .text( "修改金额必须大于运费" ) .addClass( "ui-state-highlight" );
            isT = false;
        }
	    if(isT){
...
//显示一键发货
function showThirdOneKeyDelivery(orderId,orderMType){
	$(".upt_expressInfo").text("").removeClass("ui-state-highlight");
	$("#expressNo").removeClass("ui-state-error");
	$.ajax({
		type:'get',
		url:'expressdetail.htm?orderId='+ orderId,
		success: function (data) {
			if(data != null){
				...
				$("#thirdOneKeyDelivery").modal('show');
			}
		}
	});
}
...
//一键发货
function thirdOneKeyDelivery(){
	var isT=true;
	isT = checkRegexp( $("#expressNo"),/^[0-9a-zA-Z]+$/, "物流单号必须是数字或字母" , $(".upt_expressInfo")) && isT;
	if(isT){
		$.ajax({
			type:'post',
			url:'onekeydelivery.htm',
			data: $("#thirdOneKeyDeliveryForm").serialize(),
			async: false,
			success: function(data){
				if(data == true){
					location.reload();
