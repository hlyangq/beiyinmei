...
$(function() {
	...
	$.get("getAllThirdCateForList.htm", function(data) {
		...
		for ( var i = 0; i < tCates.length; i++) {
			html += "<li><a href='javascript:;' id='cate_item" + tCates[i].catId + "' onclick='loadSecondCat("
		...
		$(".cat_second").html("");
		$(".cat_third").html("");
		if($(".cat_first").find("a").length>0){
			var cateGrade = $("#cateGrade").val();
			//如果是三级分类
			if (cateGrade == '3') {
				//查询一级分类
				$.ajax({
					url: 'queryThirdCateByCateId.htm?catId=' + $("#cateParentId").val(),
					async: false,
   ...
   /*  富文本编辑框 
	...
	$(".step_next").click(function(){
        ...
        var chk = 0;
        var chkObjs = document.getElementsByName("goodsAdded");
...
function loadSecondCat(catId, obj) {
	...
	$.get("getAllThirdCateForList.htm", function(data) {
		...
		var html = "";
		for ( var i = 0; i < tCates.length; i++) {
			html += "<li><a href='javascript:;' id='cate_item" + tCates[i].catId + "' onclick='loadThirdCat("
					+ tCates[i].catId + ",this);'>" + tCates[i].catName
					+ "</a></li>";
		}
...
function loadThirdCat(catId, obj) {
	...
	$.get("getAllThirdCateForList.htm", function(data) {
		...
		var html = "";
		for ( var i = 0; i < tCates.length; i++) {
			html += "<li><a href='javascript:;' id='cate_item" + tCates[i].catId + "' onclick='loadFourthCat("
					+ tCates[i].catId + ",this);'>" + tCates[i].catName
					+ "</a></li>";
		}
		$(".cat_third").html(html);
...
function checkBase(){
	var bValid=true;
	bValid = checkRegexp( $(".goods_no"),  /^[0-9a-zA-Z]+$/, "请输入字母或数字.", $(".goods_no_tips") ) && bValid;
...
//    }
   	...
   	bValid = checkLength( $(".goods_name"), "商品名称", $(".goods_name_tips"), 3, 120 ) && bValid;
