...
$(function () {
    ...
    $(".modi-goods").click(function () {
        ...
        $.get("queryThirdProductVoByProductId.htm?thirdProductId=" + $(this).attr("data-role"), function (data) {
            ...
            $(".goodsInfoBarcode").val(data.goodsInfoBarcode);
...
function checkProductForm() {
        $(".up_remark").val($("#sel_specDetailId").find("option:selected").text());
    var bValid = true;
    ...
    if($(".goodsInfoBarcode").val() != ""){
        bValid = checkRegexp($(".goodsInfoBarcode"), /^[0-9]+[.]{0,1}[0-9]{0,2}$/, "货品条形码只能为数字.", $(".goodsInfoBarcode_tip"))&& bValid;
    }
    ...
    if (checkSpecSymb($(".goodsInfoCostPrice"), $(".goodsInfoCostPrice_tip"))) {
        bValid = checkRegexp($(".goodsInfoCostPrice"), /^[0-9]+[.]{0,1}[0-9]{0,2}$|^\s*$/, "成本价格输入格式不正确.", $(".goodsInfoCostPrice_tip")) && bValid;
    }
    ...
    if (checkSpecSymb($(".goodsInfoMarketPrice"), $(".goodsInfoMarketPrice_tip"))) {
        bValid = checkRegexp($(".goodsInfoMarketPrice"), /^[0-9]+[.]{0,1}[0-9]{0,2}|^\s*$/, "市场价格输入格式不正确.", $(".goodsInfoMarketPrice_tip")) && bValid;
    }
    ...
    selProductParam('update_param_Tips', "flagName");
    ...
    for(var i=0;i<params.length;i++){
        $(".myup_remark_"+$(params[i]).attr("spec_id")).val($(params[i]).find("option:selected").text().trim());
    }
    if (bValid) {
        $(".sub_product").submit();
    }
