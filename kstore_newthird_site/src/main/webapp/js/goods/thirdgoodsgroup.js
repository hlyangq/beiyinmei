...
function addproduct(pageNo, pageSize)
{
    ...
    $.get("queryProductForCoupon.htm?groupId=&pageNo="+pageNo+"&pageSize="+pageSize+"&brandId="+brandId+productNo,
    function (data) 
    {
        var list = data.list;
        var productListHtml = "";
        ...
        {  
            ...
            if (list[i].specVo.length > 0)
            {
                for (var k = 0; k < list[i].specVo.length; k++)
                {
                    ...
                    productListHtml = productListHtml + ":" + list[i].specVo[k].specValueRemark + "<br/>";
                }
            }
