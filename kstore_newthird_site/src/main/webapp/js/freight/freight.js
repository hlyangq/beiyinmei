	         	   ...
	         	   if(data!=null){
                      ...
                      var arrs=s.split(",");
                       ...
                       $(".cities").each(function(){
                            ...
                            *
                            * 修改逻辑：
                            *     首先拿到当前模板已选择的所有城市id并赋值到一个数组中
                            *     当选择某个城市的时候拿到当前选择城市的id跟选择的城市id进行比对
                            *     如果存在说明该城市已经被选择就不允许再添加此城市了
                            * */
                            var yixuanCityId = new Array();
                            $("#add_tr").find("input[name='1_areas']").each(function(){
                                var tempCityIds = $(this).val().split(",")
                                for(var adc1 = 0;adc1<tempCityIds.length;adc1++){
                                    yixuanCityId.push(tempCityIds[adc1]);
                                }
                            });
                           ...
                           _this.find("a").click(function(){
                               var thisCityId =$(this).find("input").val();
                               var isCityId = false;
                               if(yixuanCityId.length > 0){
                                   for(var ab = 0;ab<yixuanCityId.length;ab++){
                                       if(yixuanCityId[ab] == thisCityId){
                                           isCityId = true;
                                           showTipAlert("该城市已经被选择过了！");
                                           break;
                                       }
                                   }
                               }
