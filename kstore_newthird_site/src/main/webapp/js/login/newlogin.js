...
function login(){
    if(checkInput()){
        var url=baseurl+"/checklogin.htm?time="+timestamp+"&username="+$("#managername").val()+"&password="+$("#managerpassword").val();//+"-"+$("#urlhide").val()
        $.ajax({
            url: url,
            ...
            success: function(data){
                ...
                if(data.status == '2'){
                    ...
                    //之前是开店流程 现在进入欢迎页
                    window.location.href=baseurl+'/goenterpage.html';
...
function checkInput(){
    ...
    $.ajax({
        url: baseurl+"/checkpatchca.htm?enterValue="+captcha,
        context: document.body,
        async:false,
        success: function(data){
          if(data==0){
