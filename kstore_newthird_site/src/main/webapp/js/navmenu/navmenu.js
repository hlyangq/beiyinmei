           ...
           for (var i = 0; i < objs.length; i++) {
	           	...
	           	if(obj.parentId== 0){
	           		...
	           		if(obj.menuVos!=null && obj.menuVos.length != 0){
						//如果模块名称为空，则是主模块的菜单
						if(obj.menuVos[0].bundleName==undefined||obj.menuVos[0].bundleName==null||obj.menuVos[0].bundleName=='') {
							var s="<li id='"+"li"+obj.id+"'><a navid='"+obj.id+"' class='nav_a' href='"+obj.menuVos[0].url+"?n="+obj.id+"&l="+obj.menuVos[0].id+"'>"+obj.designation+"</a></li>";
						} else {
							var s="<li id='"+"li"+obj.id+"'><a navid='"+obj.id+"' class='nav_a' href='"+obj.menuVos[0].bundleName+"/"+obj.menuVos[0].url+"?n="+obj.id+"&l="+obj.menuVos[0].id+"'>"+obj.designation+"</a></li>";
						}
		           		$("#indextop").append(s);
	           		}else{
						//如果模块名称为空，则是主模块的菜单
						if(obj.bundleName==undefined||obj.bundleName==null||obj.bundleName=='') {
							var s="<li id='"+"li"+obj.id+"'><a navid='"+obj.id+"' class='nav_a' href='"+obj.url+"?n="+obj.id+"'>"+obj.designation+"</a></li>";
						} else {
							var s="<li id='"+"li"+obj.id+"'><a navid='"+obj.id+"' class='nav_a' href='"+obj.bundleName+"/"+obj.url+"?n="+obj.id+"'>"+obj.designation+"</a></li>";
						}
		           		$("#indextop").append(s);
	           		}
	           	}else{
