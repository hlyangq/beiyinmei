...
public class RegisterController {
    ...
    public ModelAndView jumpProgress() {
        return new ModelAndView("/register/progress").addObject("basicSet", basicSetService.findBasicSet());
    }
    ...
    public int queryProgress(String username) {
        Customer customer = customerServiceMapper.getCustomerByInput(username);
        if(customer != null && customer.getCustomerId() != null){
            StoreInfo storeInfo = storeInfoService.selectByCustomerId(customer.getCustomerId());
            if(storeInfo != null){
                if("0".equals(storeInfo.getCheckStatus())){
                    //审核中
                    return 1;
                }else if("1".equals(storeInfo.getCheckStatus())){
                    //审核通过
                    return 2;
                }else{
                    //审核不通过
    ...
    public ModelAndView thirdRegisterCustomer(@Valid CustomerAllInfo allInfo, String varification, HttpServletRequest request) {
        ...
        if (isAddCustomerSuccess(allInfo)) {
            ...
            if (isloginSystemSuccess(request, allInfo)) {
                return new ModelAndView(CustomerConstantStr.GOENTERPAGESUCCEED);
            }
            return new ModelAndView(CustomerConstantStr.GOENTERPAGESUCCEED);
        } else {
            return new ModelAndView(new RedirectView(CustomerConstantStr.THIRDLOGIN));
