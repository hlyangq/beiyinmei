...
public class LogisticsSingleController {
    ...
    public ModelAndView toAddThirdLogisticsSingle(HttpServletRequest request) {
        ...
        mav.addObject("companys", expressInfoService.selectByStoreId(expressmap));
        // 设置要跳转的页面
        return mav;
    }
     ...
     */
    @RequestMapping("/valiLogisticsSingle")
    @ResponseBody
    public int valiLogisticsSingle(Long storeId){
        int result = expressInfoService.selectDefaultRows(storeId);
        if(result > 0){
            return 1;
        }else{
            return 0;
