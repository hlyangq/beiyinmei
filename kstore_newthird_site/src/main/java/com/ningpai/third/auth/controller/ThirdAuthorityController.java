...
public final class ThirdAuthorityController {
    ...
    public ModelAndView modifyEmpToDisable(HttpServletRequest request, Long custId, String flag) {
        ...
        if(request.getSession().getAttribute("customerId") != null){
            //要停用的员工信息
            CustomerAllInfo customer = customerServiceMapper.selectByPrimaryKey(custId);
            //操作员工信息（当前登录的员工）
    ...
    public ModelAndView updateBycustId(HttpServletRequest request, ThirdManagerAuthority author) {
        ...
        if(request.getSession().getAttribute("customerId") != null){
            //要修改的会员
            CustomerAllInfo customer = customerServiceMapper.selectByPrimaryKey(author.getManagerId());
            //操作员工信息（当前登录的员工）
            CustomerAllInfo customerAllInfo = customerServiceMapper.selectByPrimaryKey((Long) request.getSession().getAttribute("customerId"));
            if(customerAllInfo != null && customerAllInfo.getCustomerUsername() != null){
                // 操作日志
    ...
    public ModelAndView delCustomer(HttpServletRequest request, String[] custId) {
        ...
        if(request.getSession().getAttribute("customerId") != null){
            //要删除的员工信息
            CustomerAllInfo customer = customerServiceMapper.selectByPrimaryKey(Long.valueOf(custId[0]));
            //操作员工信息（当前登录的员工）
            CustomerAllInfo customerAllInfo = customerServiceMapper.selectByPrimaryKey((Long) request.getSession().getAttribute("customerId"));
            if(customerAllInfo != null && customerAllInfo.getCustomerUsername() != null){
                // 操作日志
    ...
    public int delAuthority(HttpServletRequest request, ThirdAuthority thirdAuthority) {
        ThirdAuthority detail = thirdAuthorityService.selectAuthorById(thirdAuthority.getId());
        if(request.getSession().getAttribute("customerId") != null){
            CustomerAllInfo customerAllInfo = customerServiceMapper.selectByPrimaryKey((Long) request.getSession().getAttribute("customerId"));
            if(customerAllInfo != null && customerAllInfo.getCustomerUsername() != null){
                // 操作日志
