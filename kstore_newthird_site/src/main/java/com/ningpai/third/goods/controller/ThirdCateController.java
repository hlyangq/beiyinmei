...
public class ThirdCateController {
    ...
    private void filterXss(ThirdCate cate)
    {
        if (null != cate)
        {
            cate.setCatName(stripXSS(cate.getCatName()));
            cate.setCatSeoTitle(stripXSS(cate.getCatSeoTitle()));
            cate.setCatSeoKeyword(stripXSS(cate.getCatSeoKeyword()));
            cate.setCatSeoDesc(stripXSS(cate.getCatSeoDesc()));
        }
    }
    ...
    public ModelAndView delThirdCate(Long thirdCateId,
            HttpServletRequest request) {
        ThirdCateVo thirdCateVo = thirdCateService.queryThirdCateById(thirdCateId);
        // 根据商品分类ID删除分类信息
    ...
    public ModelAndView updateThirdCate(ThirdCate cate,
        ...
        filterXss(cate);
        //修改前的商品分类
        ThirdCateVo thirdCateVo = thirdCateService.queryThirdCateById(cate.getCatId());
        //添加操作日志
        if(request.getSession().getAttribute(ThirdValueBean.CUST) != null){
            Customer cust = (Customer) request.getSession().getAttribute(ThirdValueBean.CUST);
            if(cust.getCustomerUsername() != null){
