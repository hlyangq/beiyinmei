...
public class ImageManagerThirdController {
    ...
    public ModelAndView saveImageAction(MultipartHttpServletRequest request, InfoImageManage infoImageManage) {
        ...
        if (muFile.getSize() > 0) {
            ...
            //保存图片添加操作日志
            if(infoImageManageService.saveInfoImageManage(infoImageManage) > 0){
                if(request.getSession().getAttribute(ThirdValueBean.CUST) != null){
                    ...
                    if(cust.getCustomerUsername() != null){
                        // 操作日志
                        OperateLogUtil.addOperaLog(request, cust.getCustomerUsername(), "添加图片", "添加图片-->用户名：" + cust.getCustomerUsername());
                        LOGGER.info("添加图片");
                    }
                }
            }
