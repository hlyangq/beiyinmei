...
public class CouponThirdController {
    ...
    public int newDelCouponThird(Long couponId, HttpServletRequest request) {
        ...
        if(request.getSession().getAttribute(ThirdValueBean.CUST) != null){
            ...
            if(cust.getCustomerUsername() != null){
                // 操作日志
                OperateLogUtil.addOperaLog(request, cust.getCustomerUsername(), "删除优惠券", "删除优惠券，名称【" + coupon.getCouponName() + "】-->用户名：" + cust.getCustomerUsername());
                LOGGER.info("删除优惠券");
            }
        }
        // 执行删除优惠券操作
    ...
    public int newdelCouponList(Long[] couponId, HttpServletRequest request) {
        ...
        for (int i = 0; i < couponId.length; i++) {
            Coupon coupon = couponService.searchCouponById(couponId[i]);
            names = names.concat("【"+coupon.getCouponName()+"】");
        }
        //添加操作日志
        if(request.getSession().getAttribute(ThirdValueBean.CUST) != null){
            Customer cust = (Customer) request.getSession().getAttribute(ThirdValueBean.CUST);
            if(cust.getCustomerUsername() != null){
                // 操作日志
                OperateLogUtil.addOperaLog(request, cust.getCustomerUsername(), "批量删除优惠券", "批量删除优惠券，名称" + names + "-->用户名：" + cust.getCustomerUsername());
                LOGGER.info("批量删除优惠券");
            }
        }
        // 执行删除优惠券操作
