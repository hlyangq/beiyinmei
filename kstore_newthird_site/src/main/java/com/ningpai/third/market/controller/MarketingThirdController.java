...
public class MarketingThirdController {
    ...
    public int newDelMarketingById(Long marketingId, HttpServletRequest request) {
        ...
        if (null != marketing) {
            ...
            if(request.getSession().getAttribute(ThirdValueBean.CUST) != null){
                ...
                if(cust.getCustomerUsername() != null){
                    // 操作日志
                    OperateLogUtil.addOperaLog(request, cust.getCustomerUsername(), "删除促销", "删除促销，促销名称【" + marketing.getMarketingName() + "】-->用户名：" + cust.getCustomerUsername());
                    LOGGER.info("删除促销");
                }
            }
            // 根据ID删除单个的促销信息
    ...
    public ModelAndView newEditMarketing(Long marketingId, String marketingFlag, HttpServletRequest request) {
        ...
        Marketing mark = marketingService.marketingDetailNotTime(marketingId);
        if(thirdId !=null && mark.getBusinessId() != null){
            if(!thirdId.equals(mark.getBusinessId())){
                return new ModelAndView(new RedirectView(request.getContextPath() + "/sellerinfo.html"));
            }
        }
        if(mark != null && "1".equals(mark.getIsAll())&&"15".equals(mark.getCodexType())) {
            //折扣促销批量折扣
            if (!CollectionUtils.isEmpty(mark.getPreDiscountMarketings())) {
                dis = mark.getPreDiscountMarketings().get(0).getDiscountInfo();
            }
        }
        // 修改促销页面
