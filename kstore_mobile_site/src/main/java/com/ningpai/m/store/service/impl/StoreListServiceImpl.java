            ...
            if (CollectionUtils.isNotEmpty(storeInfos)) {
                ...
                for (int i = 0; i < storeInfos.size(); i++) {
                    ...
                    if (null != storeInfo.getStoreId() && null != customerId) {
                        Integer t_collection = this.storeListMapper.IsCollection(storeInfo.getStoreId(), customerId);
                        storeInfo.setIsCollection(t_collection == null ? 0 : t_collection);
                    }

                    //根据店铺ID获取关注粗
                    if(null != storeInfo.getStoreId()){
                        Integer t_collection = this.storeListMapper.IsCollection(storeInfo.getStoreId(), null);
