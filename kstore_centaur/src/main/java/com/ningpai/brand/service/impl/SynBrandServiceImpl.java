    ...
    public ReturnObject SynchronousGoodsBrand() {
        ...
        if (dlist != null && dlist.length != 0) {
            ...
            for (int i = 0; i < dlist.length; i++) {
                ...
                // 检测品牌是否存在
                int brandcheck = synBrandMapper.checkBrand(Long.valueOf(brand.getBrand_code()));
                // 定义商品品牌对象
                GoodsBrand gb = new GoodsBrand();
                // 品牌id设值
                gb.setBrandId(Long.valueOf(brand.getBrand_code()));
                // 品牌名称设值
                gb.setBrandName(brand.getBrand_name());
                // 品牌昵称设值
                gb.setBrandNickname(brand.getBrand_name());
                // 品牌为删除
                gb.setBrandDelflag("0");
                // 品牌SEOTitle设值
                gb.setBrandSeoTitle(brand.getBrand_name());
                // 品牌SeoKeyword设值
                gb.setBrandSeoKeyword(brand.getBrand_name());
                // 品牌SEodesc设值
                gb.setBrandSeoDesc(brand.getBrand_name());
                // 品牌排序
                gb.setBrandSort(0);
                // 判断品牌是否存在
                if (brandcheck == 0) {
