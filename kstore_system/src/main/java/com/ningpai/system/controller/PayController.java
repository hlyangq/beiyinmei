...
//        return new ModelAndView("jsp/system/payset", "pb", payService.findByPageBean(pb, selectBean));
        return new ModelAndView("jsp/customer/paymentMode", "pb", payService.findByPageBean(pb, selectBean));
    }
    ...
    public Map updatePay(MultipartHttpServletRequest request, @Valid Pay pay) {
        ...
        try {
            ...
            result = payService.updatePay(pay);

            //===========修改np_sys_pay的货到付款开关，关联到np_sys_payment数据表
            Pay tpay = payService.findByPayId(pay.getPayId());
            String payType = "";
            String isOpen = pay.getIsOpen();
            if(tpay != null){
                payType = tpay.getPayType();
            }
            //货到付款的情况,货到付款payType=6,修改np_sys_payment数据表控制开关
            if("6".equals(payType)){
                Payment payment = new Payment();
                payment.setIsOpen(isOpen);
                payment.setPaymentId(2l);//货到付款的id
