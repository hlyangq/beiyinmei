...
public class EmailServerController {
    ...
    public ModelAndView emailSet(HttpServletRequest request,
        ...
        //邮箱验证信息
        MessageBean validateInfo = messageService.selectById(11);
        //绑定邮箱信息
        MessageBean bindInfo = messageService.selectById(13);
        //找回密码信息
        MessageBean findInfo = messageService.selectById(3);
        map.put("validateInfo",validateInfo);
        map.put("bindInfo",bindInfo);
        map.put("findInfo",findInfo);
        return new ModelAndView("jsp/system/emailserver", "server",
                emailServerService.findServer()).addObject("flag", flag).addObject("map",map);
    }
    ...
    public ModelAndView updateMail(HttpServletRequest request,
            HttpServletResponse response, @Valid EmailServer emailServer,MessageBean messageBean,
        ...
        try {
            ...
            //修改消息设置
            pr.print(messageService.updateInformSelective(messageBean));
            // 获取session中的用户名
