    ...
    public static void exportProductList(List<Object> productList,ProductWareService productWareService, HttpServletResponse response) {
        ...
        if (null != productList && !productList.isEmpty()) {
            ...
            for (int i = 0; i < productList.size(); i++) {
                ...
                //根据货品id查询到仓库库存
                List<ProductWare> productWareList = productWareService.queryAllInfoByProductId(product.getGoodsInfoId());
                if(productWareList != null ){
                    if(productWareList.size() == 1){
                        tempRow.createCell(3).setCellValue(productWareList.get(0).getWareStock());
                    }else{
