...
public class GoodsSpecController {
        ...
        try {
            ...
            OperaLogUtil.addOperaLog(request, (String) request.getSession().getAttribute(ValueUtil.NAME), ValueUtil.UPDATESPECINFO,
                    (String) request.getSession().getAttribute(ValueUtil.OPERAPATH) + LOGGERINFO3 + goodsSpec.getSpecName() + LOGGERINFO4
                            + (String) request.getSession().getAttribute(ValueUtil.NAME));
            // 返回结果
            return new ModelAndView(new RedirectView(PathUtil.ALLSPECCONTROLLER + ValueUtil.TOKENPARAM + CSRFTokenManager.getTokenFromRequest(request) + "&pageNo=" + pb.getPageNo() + "&pageSize=" + pb.getPageSize()));
        } finally {
    ...
    public int isSpecValueCanDelete(Long specvalueId) {
        ...
        return goodsSpecService.isSpecValueCanDelete(list) ? 0 : 1;
    }
     ...
     * @return 可以删除返回0  不能删除返回1
     */
    @RequestMapping("/isspeccandelete")
    @ResponseBody
    public int isSpecCanDelete(Long[] specIds) {
        return goodsSpecService.isSpecCanDelete(specIds) ? 0 : 1;
    }
     ...
     * @return 可以删除返回0  不能删除返回1
     */
    @RequestMapping("/isspeccandeleteone")
    @ResponseBody
    public int isSpecCanDeleteOne(Long specId) {
        Long [] ids = {specId};
        return goodsSpecService.isSpecCanDelete(ids) ? 0 : 1;
    }
