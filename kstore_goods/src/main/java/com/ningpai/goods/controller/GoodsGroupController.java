...
public class GoodsGroupController {
    ...
    public ModelAndView delGroup(Long groupId, HttpServletRequest request) {
        ...
        Long[] ids = {groupId};
        goodsGroupService.deleteShoppingCartByFitIds(ids);
    ...
    public ModelAndView batchDelGroup(Long[] groupIds, HttpServletRequest request) {
        ...
        // 删除购物车中的记录
        goodsGroupService.deleteShoppingCartByFitIds(groupIds);
        // 记录当前登录者的操作日志
    ...
    public ModelAndView saveGroup(@Valid GoodsGroup goodsGroup, HttpServletRequest request) {
        ...
        // 添加一个商品组合
        goodsGroupService.saveGoodsGroup(goodsGroup, (String) request.getSession().getAttribute(ValueUtil.NAME));
        // 非空验证 商品组合名称 和是否添加成功
    ...
    public ModelAndView queryByPrimaryKey(Long groupId) {
        ...
        GoodsGroupVo groupVo = goodsGroupService.queryVoByPrimaryKey(groupId);
        List<GoodsBrand> brandList = goodsBrandService.queryAllBrand();
        return new ModelAndView(PathUtil.GROUPRELEPRODUCTVIEW)
                .addObject("groupVo", groupVo)
                .addObject("basicAddress", basicAddress)
                .addObject("brandList", brandList)
                .addObject("oneCateList", oneCateList);
