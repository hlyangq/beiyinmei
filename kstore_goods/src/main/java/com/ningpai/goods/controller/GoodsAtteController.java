...
public class GoodsAtteController {
    ...
    public ModelAndView findAllGoodsArrival(PageBean pageBean, SelectBean selectBean) {
        ...
        LOGGER.info(ValueUtil.FINDALLGOODSATTEINFO);
        // 返回结果
        return new ModelAndView(ValueUtil.GOODSARRIVALN).addObject("map", map);
    }
    ...
    public ModelAndView findGoodsAtteByProductIdArrival(Long productId, PageBean pageBean) {
        ...
        LOGGER.info(ValueUtil.FINALGOODSATTEINFO);
        // 返回结果
        return new ModelAndView(ValueUtil.PRODUCARRIVALN).addObject("pb", this.goodsAtteService.querybyProductIdArrival(pageBean, productId));
    }
