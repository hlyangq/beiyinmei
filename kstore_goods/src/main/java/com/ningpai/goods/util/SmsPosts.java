...
public class SmsPosts {
    ...
    public static boolean sendPost(Sms sms, GoodsProduct goodsProduct, String mobile) throws IOException {
        ...
        if (null != goodsProduct.getOfollowPrice()) {
            param +=  "您所关注的" + goodsProduct.getDisName() + "地区的，商品为：" + goodsProduct.getGoodsInfoName()
                    + "的商品，由" + goodsProduct.getOfollowPrice() + "元,降到了" + goodsProduct.getNfollowPrice() + "元,快来选购吧" ;
        } else {
            param +=  "您好," + basicSetService.findBasicSet().getBsetName() + "提醒您,我们的" + goodsProduct.getGoodsInfoName() + "商品降价了" ;
        }
     ...
     * @throws IOException
     */
    public static boolean sendANPost(Sms sms, GoodsProductVo goodsProductVo, String[] mobile) throws IOException {
        // 调用zyer.cn的短信接口
        String param= "";
        if(goodsProductVo != null){
            param += "您所关注的商品" + goodsProductVo.getGoodsInfoName() + "已经到货啦快来选购吧" ;
        }else{
            param += "您所关注的商品已经到货啦快来选购吧" ;
        }
        sms.setMsgContext(param);
        return MSMSendUtil.sendMsm("", sms.getLoginName(), sms.getPassword(), mobile, sms.getMsgContext(),sms.getSmsKind(),sms.getHttpUrl());
    }
