...
public class GoodsAtteServiceImpl implements GoodsAtteService {
    ...
    public PageBean queryByPageAndParam(PageBean pb, SelectBean selectBean) {
        ...
        Map<String, Object> map = new HashMap<String, Object>();
        try {
            map.put(ValueUtil.CONDITION, "".equals(selectBean.getCondition()) ? null : selectBean.getCondition());
            map.put(ValueUtil.SEARCHTEXT, "".equals(selectBean.getSearchText()) ? null : selectBean.getSearchText());
    ...
    public PageBean queryByPageAndParamArrival(PageBean pb, SelectBean selectBean) {
        ...
        try {
            map.put(ValueUtil.CONDITION, "".equals(selectBean.getCondition()) ? null : selectBean.getCondition());
            map.put(ValueUtil.SEARCHTEXT, "".equals(selectBean.getSearchText()) ? null : selectBean.getSearchText());
            pb.setRows(goodsProductMapper.queryTotalCountArrival(map));
            map.put(ValueUtil.STARTROWNUM, pb.getStartRowNum());
            map.put(ValueUtil.ENDROWNUM, pb.getEndRowNum());
            pb.setList(goodsProductMapper.queryByParamArrival(map));
            return pb;
        } finally {
            map = null;
        }
