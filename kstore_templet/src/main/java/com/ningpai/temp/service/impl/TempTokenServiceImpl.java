...
public class TempTokenServiceImpl implements TempTokenService {
    ...
    public TempToken selectTokenByType(String temp1) {
        ...
        TempToken tempToken = (TempToken) redisAdapter.get(CacheKeyConstant.TEMPTOKEN_KEY);

        // 缓存中有token 直接返回缓存中的token
        if (null != tempToken) {
            return tempToken;
        }

        tempToken = tokenMapper.selectTokenByType(temp1);

        // 将token放入缓存
        redisAdapter.put(CacheKeyConstant.TEMPTOKEN_KEY, tempToken);
