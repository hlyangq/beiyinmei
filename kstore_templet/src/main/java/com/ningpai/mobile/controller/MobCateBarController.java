...
//        map.put("channelAdvers", this.mobCateBarService.selectStoreListImage(null));
        return new ModelAndView(MOB_STORELIST_STREET).addObject("pb", mobCateBarService.selectStoreListImageByPages(pb, null));
    }
    ...
    public ModelAndView storelistqianggouimage(PageBean pb) {
        ...
        return new ModelAndView("jsp/mobile/mob_storelist_qianggou").addObject("pb", pb);
    }
    ...
    public ModelAndView queryMobCateBarByPb(PageBean pb) {
        mobCateBarService.selectMobCateBarByPb(pb);
        return new ModelAndView("jsp/mobile/mobcatebar_list", "pb", pb);
    }
    ...
    public ModelAndView showMobCateBar(Long mobCateBarId,HttpServletRequest request) {
        ...
        // 修改
        String pageNo = request.getParameter("pageNo");//进入点页号
        if (null != mobCateBarId) {
            MobCateBar cate=this.mobCateBarService.selectMobcateBarById(mobCateBarId);
            mav.addObject("mobcatebar", cate);
            mav.addObject(GRADE, cate.getGrade());
            mav.addObject("pageNo", pageNo);//记录页面
        }else{
    ...
    public ModelAndView showMobCateBarByParentId(Long mobCateBarId, Long parentId,HttpServletRequest request) {
        ...
        mav.addObject(GRADE, parent.getGrade()+1);
        mav.setViewName("jsp/mobile/show_mobcatebar");
        mav.addObject("pageNo", pageNo);//记录页面
        return mav;
    }
    ...
    public ModelAndView updateMobCateBar(@Valid MobCateBar mobCateBar, BindingResult bindingResult, MultipartHttpServletRequest request) {
        ...
        return new ModelAndView(new RedirectView(QUERY_MOB_CATE_BAR_BY_PB+"?pageNo="+pageNo));
    }
