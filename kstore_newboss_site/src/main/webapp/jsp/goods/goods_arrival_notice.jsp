...
<div class="page_body container-fluid">
    <div class="row">
        ...
        <div class="col-lg-20 col-md-19 col-sm-18 main">
            ...
            <%--<div class="filter_area">--%>
              <%--<form role="form" class="form-inline" action="findAllGoodsArrival.htm" method="post" id="searchForm">--%>
            ...
            <tbody>
            <c:forEach items="${map.pb.list }" var="goodsProduct" varStatus="sta">
                <%--<c:if test="${goodsAtte.product.goodsInfoItemNo gt 0}">--%>
                    <tr>
                        <td>
                          <div class="data_item">
                            <img height="60" src="${goodsProduct.goodsInfoImgId }"/>
                                <p title="${goodsProduct.goodsInfoName}"><c:if test="${fn:length(goodsProduct.goodsInfoName)>40}">${fn:substring(goodsProduct.goodsInfoName, 0, 40)}</c:if>
                                    <c:if test="${fn:length(goodsProduct.goodsInfoName)<=40}">${goodsProduct.goodsInfoName}</c:if>
                                </p>
                                <p class="text-muted"> ${goodsProduct.goodsInfoPreferPrice}</p>
                          </div>
                        </td>
                        <td>${goodsProduct.goodsInfoItemNo}</td>
                        <td> ${goodsProduct.goodsInfoCostPrice}</td>
                        <td>${goodsProduct.goodsInfoMarketPrice}</td>
                        <td>
                            <button type="button" class="btn btn-default" onclick="viewAtte(${goodsProduct.goodsInfoId },'${goodsProduct.goodsInfoName}')">查看</button>
                        </td>
                    </tr>
                <%--</c:if>--%>
            </c:forEach>
    ...
    <script>
      ...
      function viewAtte(goodsInfoId,name){  
          $("#nameTitle").text(name);   
          $("#goodsAtteDetail").attr('src','findGoodsAtteByProductIdArrival.htm?CSRFToken=${token}&productId='+goodsInfoId);
