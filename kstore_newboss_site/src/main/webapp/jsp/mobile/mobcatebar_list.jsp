...
<html lang="zh-cn">
  ...
  <body>
        ...
        <div class="col-lg-20 col-md-19 col-sm-18 main">
          ...
          <div class="main_cont">
            ...
            <div class="common_data p20">
              ...
              <div class="data_ctrl_area mb20">
                ...
                <div class="data_ctrl_brns pull-left">
                  <button type="button" class="btn btn-info" onclick="createMobCateBar();">
                    ...
                    <button type="button" class="btn btn-info btn-expand">全部收起</button>
                    <%--<button type="button" class="btn btn-info btn-expand" onclick="collapseAll();">全部收起</button>--%>
              ...
              <!-- 这是另一种表格，带伸缩，不带分页 -->
              <table class="treetable table table-striped table-hover" cellspacing="0" width="100%" id="treeTable">
              <thead>
              <tr>
                <th style="width: 30%;padding-left: 80px;text-align: left;">分类名称</th>
                <th>分类图片</th>
                <th>是否显示</th>
                <th width="150">操作</th>
              </tr>
              ...
              <tbody id="treeable">
               ...
               <input type="hidden" value="<%=basePath %>queryMobCateBarByPb.htm" id="formName">
               <form role="form" action="queryMobCateBarByPb.htm" method="post"  id="adv_form" class="form-inline"></form>
               <c:forEach var="mobcatebar" items="${pb.list }" varStatus="sta">
             <c:set var="count" value="${count+1 }"></c:set>
          
              <tr data-sort="${count}" id="sort_${mobcatebar.cateBarId }" class="sortIndex" data-tt-id="${mobcatebar.cateBarId}">
                ...
                <td>
                	...
                	<c:if test="${mobcatebar.isUsing=='0'}"><span class="label label-default">不显示</span></c:if>
					<c:if test="${mobcatebar.isUsing=='1'}"><span class="label label-success">显示</span></c:if>
					</a>
				</td>
                <td>
                  <div class="btn-group">
              ...
              <c:if test="${fn:length(mobcatebar.childs)>0}">
								<c:forEach var="childcatebar" items="${mobcatebar.childs }" varStatus="status">
								   <c:set var="count" value="${count+1 }"></c:set>
									  <tr  data-tt-id="${childcatebar.cateBarId}" data-tt-parent-id="${childcatebar.parentId}" class="sortIndex" id="sort_${childcatebar.cateBarId }" >
						                <td style="text-align:left;">
						                ...
						                <td>
						                	...
						                	<c:if test="${childcatebar.isUsing=='0'}"><span class="label label-default">不显示</span></c:if>
											<c:if test="${childcatebar.isUsing=='1'}"><span class="label label-success">显示</span></c:if>
											</a>
										</td>
						                <td>
						                  <div class="btn-group">
						                    ...
						                    <ul class="dropdown-menu" role="menu">
                                                <c:if test="${childcatebar.grade=='2'}">
                                                    <li> <a href="javascript:;" onclick="createChild(${childcatebar.cateBarId })">添加子分类</a></li>
                                                </c:if>
                                    ...
                                    <c:if test="${fn:length(childcatebar.childs)>0}">
                                      <c:forEach var="childcatebar" items="${childcatebar.childs }" varStatus="status">
                                          <c:set var="count" value="${count+1 }"></c:set>
                                          <tr data-tt-id="${childcatebar.cateBarId}" data-tt-parent-id="${childcatebar.parentId}" class="sortIndex " id="sort_${childcatebar.cateBarId }" >
                                              <td style="text-align:left;">
                                                      ${childcatebar.name}</td>
                                              ...
                                              <td>
                                                  <div class="btn-group">
                                                      <button type="button" class="btn btn-default" onclick="updateMobCateBar(${childcatebar.cateBarId });">编辑</button>
                                                      <button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown">
                                                          <span class="caret"></span>
                                                          <span class="sr-only">Toggle Dropdown</span>
                                                      </button>
    ...
    <script src="<%=basePath %>js/jquery-treetable-ajax-persist/src/javascripts/jquery.treetable-ajax-persist.js"></script>
    <script src="<%=basePath %>js/jquery-treetable-ajax-persist/src/javascripts/jquery.treetable-3.0.0.js"></script>
    <script src="<%=basePath %>js/jquery-treetable-ajax-persist/src/javascripts/persist-min.js"></script>
    <link href="<%=basePath %>js/jquery-treetable-ajax-persist/src/stylesheets/jquery.treetable.css" media="all" rel="stylesheet" type="text/css" />
    ...
    <script>
      ...
      function updateMobCateBar(id){
          ...
          location="showMobCateBar.htm?CSRFToken=${token}&mobCateBarId="+id+"&pageNo="+pageNo;
    ...
    //修改二级分类导航  
      function updateChildMobCateBar(id,parentId){
          debugger;
          var pageNo = '${pb.pageNo}'
      	location="showMobCateBarByParentId.htm?CSRFToken=${token}&mobCateBarId="+id+"&parentId="+parentId+"&pageNo="+pageNo;;
      }
    </script>
  <script>
