						              ...
						              <tr>
                                          <th>
                                              <div class="pull-right">
                                                  ...
                                                  <a class="btn btn-info" href="javascript:void(0);" onclick="yichus(0);">抹分角</a>
                                              </div>

                                              <div class="pull-left form-inline">
                                                  <label>批量折扣:</label>
                                                  <input type="text" class="form-control" style="width:60px;" onblur="plzhekou(this);"/>
          ...
          <div class="modal-body">
            <div class="mb10">
                <form class="form-inline" id="searchGoodsInfo" action="" method="post" >
                    ...
                    <input type="hidden" name="marketType" value="15"/>
                <div class="form-group">
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            <div class="modal-article">
              ...
              <img src="<%=basePath %>/images/syshelp/zhekou.png" alt="">
  	...
  	/*点击添加货品的时候*/ 
		...
		function addpro(obj){
		    ...
		    if(obj.checked==true){
		    	...
		    	htm+='<td  width="20%">'+goodsName+'</td>';
		    	htm+='<td  width="6.5%" class="goodsPrices">'+goodsPrice+'</td>';
		    	...
		    	htm+='<td  width="6.5%"><span class="zhj">'+goodsPrice+'</span><input type="hidden" class="form-control" name="discountPrice" /><input type="hidden" class="form-control" name="discountFlag" id="discount_prices_'+productId+'" value="0" readonly  ></td>';
		    	htm+='<td width="22%"><input type="button" onclick="mofensingle('+goodsPrice+',this,1);" value="移除分" class="btn btn-sm btn-default btn-min"/>';
		    	htm+='<input type="button" onclick="mofensingle('+goodsPrice+',this,0);" value="移除分角" class="btn btn-sm btn-default btn-min"/>';
		    	htm+='<button onclick="removeTr(this);" class="btn btn-sm btn-default btn-min">移除</button></td>';
		  	  	htm+="</tr>";
		    $("#readproduct tbody").append(htm);
		    }else{
		      $("#goods_tr_"+productId).remove();a
		...
		function mofensingle(price,obj,fixed){
		  ...
		  var obp = $(obj).parents("tr").find("input[name='discountInfo']").val();
            var dianQ2 = (price*obp)+'';
            dianQ2 = Number(dianQ2).toFixed(2);
            var hyc  = dianQ2.length;
            var b = 0;
		  if(fixed==1){
              b = dianQ2.substring(0,hyc-1);
	 	 	 	$(obj).parents("tr").find("input[name='discountFlag']").val(1); 
		  }else if(fixed==0){
              b = dianQ2.substring(0,hyc-3);
