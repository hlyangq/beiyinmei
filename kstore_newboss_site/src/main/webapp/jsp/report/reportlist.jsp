                ...
                <div class="common_data p20">
                    ...
                    <table class="table table-striped table-hover">
                        ...
                        <c:forEach items="${pb.list}" var="report" varStatus="i">
                            <fmt:formatDate value="${report.startTime }" pattern="yyyy-MM-dd HH:mm:ss" var="startTime" />
                            <fmt:formatDate value="${report.endTime }" pattern="yyyy-MM-dd HH:mm:ss" var="endTime" />
                            <tr>
                                <td ><input type="checkbox" name="storeIds"  value="${report.storeId }"></td>
                                <td><fmt:formatDate type="date" value="${report.startTime }" pattern="yyyy-MM-dd"/>~<fmt:formatDate type="date" value="${report.endTime }" pattern="yyyy-MM-dd"/></td>
                                <td >${report.storeName }</td>
                                <td><fmt:formatNumber type="number" value="${report.totalOrderMoney}"    maxFractionDigits="2"/></td>
                                <td><fmt:formatNumber type="number" value="${report.orderPrice}" maxFractionDigits="2"/></td>
                                <td><fmt:formatNumber type="number" value="${report.cateRatePrice}" maxFractionDigits="2"/></td>
                                <td><fmt:formatNumber type="number" value="${report.cateRatebackPrice}" maxFractionDigits="2"/></td>
                                <td><fmt:formatNumber type="number" value="${report.refundPrice}" maxFractionDigits="2"/></td>
                                <td>
                                    ...
                                    <c:if test="${report.settleStatus==1}">
                                        <span class="label label-danger">已结算</span>
                                    </c:if>
                                </td>
                                ...
                                <td>
                                    <div class="btn-group">
                                        <button type="button" class="btn btn-default" onclick="window.location.href='queryReportOrderCate.htm?CSRFToken=${token }&storeId=${report.storeId }&startTime=${startTime }&endTime=${endTime }'">收入详情</button>
                                        <button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown">
                                            <span class="caret"></span>
                                            <span class="sr-only">Toggle Dropdown</span>
                                        </button>
                                        <ul class="dropdown-menu" role="menu">
                                            <li><a href="javascript:;" onclick="window.location.href='queryReportBackOrderCate.htm?CSRFToken=${token }&storeId=${report.storeId }&startTime=${startTime }&endTime=${endTime }'">退单详情</a></li>
                                            <li><a href="javascript:;" onclick="doSettlement('${report.storeId}','${startTime }','${endTime }');">设为已结算</a></li>
                                        </ul>
                                    </div>
                                </td>
                            </tr>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <form class="form-horizontal" action="generatReport.htm" method="post" id="genReportForm">
                ...
                <h4 class="modal-title">更新对账信息</h4>
                </div>
