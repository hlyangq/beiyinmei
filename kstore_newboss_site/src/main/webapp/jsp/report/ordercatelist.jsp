            ...
            <div class="main_cont">
                ...
                <div class="common_data p20">
                    ...
                    <table class="table table-striped table-hover">
                        ...
                        <tbody>
                    <c:forEach items="${pb.list}" var="order" varStatus="i">
                        <fmt:formatDate value="${order.startTime }" pattern="yyyy-MM-dd" var="startTime" />
                        <fmt:formatDate value="${order.endTime }" pattern="yyyy-MM-dd" var="endTime" />
                        <tr>
                            <td><input type="checkbox" name="reportIds"  value="${order.orderId }"></td>
                            <td>${order.orderCode }</td>
                            <td><fmt:formatNumber value="${order.orderPrice }"  pattern="0.00"/> </td>
                            <td><fmt:formatNumber value="${order.orderOldPrice }"  pattern="0.00"/></td>
                            <td><fmt:formatNumber value="${order.expressPrice }"  pattern="0.00"/></td>
                            <td><fmt:formatNumber value="${order.orderCatePrice }"  pattern="0.00"/></td>
                            <td>
                                <button type="button" class="btn btn-default" onclick="window.location.href='queryReportOrderCateDetail.htm?CSRFToken=${token }&storeId=${report.storeId }&orderId=${order.orderId }'">查看明细</button>
                            </td>
                        </tr>
                        </c:forEach>
                        </tbody>
