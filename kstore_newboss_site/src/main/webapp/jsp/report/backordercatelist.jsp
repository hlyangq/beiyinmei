            ...
            <div class="main_cont">
                ...
                <div class="common_data p20">
                    ...
                    <table class="table table-striped table-hover">
                        ...
                        <tbody>
                    <c:forEach items="${pb.list}" var="backOrder" varStatus="i">
                        <fmt:formatDate value="${backOrder.startTime }" pattern="yyyy-MM-dd" var="startTime" />
                        <fmt:formatDate value="${backOrder.endTime }" pattern="yyyy-MM-dd" var="endTime" />
                        <fmt:formatDate value="${backOrder.backTime }" pattern="yyyy-MM-dd" var="backTime" />
                        <tr>
                            <td><input type="checkbox" name="reportIds"  value="${backOrder.backOrderId }"></td>
                            <td>${backOrder.backOrderCode }</td>
                            <td>${backOrder.orderCode }</td>
                            <td><fmt:formatNumber value="${backOrder.backPrice }"  pattern="0.00"/> </td>
                            <td><fmt:formatNumber value="${backOrder.backOrderCatePrice }"  pattern="0.00"/> </td>
                            <td>${backTime}</td>
                            <td>

                                <c:if test="${backOrder.backCheck=='4'}">
                                    <a href="javascript:;"><span class="label label-success">退货</span></a>
                                </c:if>
                                <c:if test="${backOrder.backCheck==10}">
                                    <a href="javascript:;"><span class="label label-danger">退款</span></a>
                                </c:if>
                            </td>
                            <td>
                                <button type="button" class="btn btn-default" onclick="window.location.href='queryReportBackOrderCateDetail.htm?CSRFToken=${token }&storeId=${report.storeId }&orderId=${backOrder.orderId }'">查看明细</button>
                            </td>
                        </tr>
                        </c:forEach>
                        </tbody>
