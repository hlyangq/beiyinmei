...
<script>
    ...
    $(function () {
        ...
        $("#imgUpload").change(function(){
            ...
            //验证文件格式
            var suffixArray = $.trim($("#suffixArray").val());
            suffixArray = suffixArray.toLowerCase();
            var arr_suffixArray = suffixArray.split(",");

            for(var i = 0;i<this.files.length;i++){
                var fileName = this.files[i].name;
                var photoExt=fileName.substr(fileName.lastIndexOf(".")+1).toLowerCase();
                var isFlag = false;
                for(var j in arr_suffixArray){
                    if(arr_suffixArray[j] == photoExt){
                        isFlag = true;
                    }
                }
                if(!isFlag){
                    art.dialog.alert('请上传'+suffixArray+'格式的图片!');
                    return false;
                }
            }

            imgModule.button({name: '保存',disabled: true});
            console.log("upload imgs : " + this.files.length);
    ...
    {
        /*AJAX查询分类信息*/
        $.get("ajaxQueryImageForChoose.htm?classifyId="+$("#search_classifyId").val()+"&pageNo=" + pageNo + (pageSize==undefined?"":("&pageSize=" + pageSize))+"&CSRFToken="+$("#CSRFToken").val(),
                function (data)
                {
                    //设置条数
    ...
    function callback(msg){
        ...
        if(msg.imgurl=="-1"){
            art.dialog.alert('请上传图片！');
        }else if(msg.imgurl=="-2"){
            art.dialog.alert('上传图片不得大于'+ msg.maxsize +'！');
        }else if(msg.imgurl=="-3"){
            art.dialog.alert('请上传gif,jpg,jpeg,png,bmp,ico格式的图片!');
        }else{
            var n = msg.imgurl.indexOf(",");
            if(n>-1){
                var imgs = msg.imgurl.split(",");
                for(var i = 0;i<imgs.length;i++){
                    if(imgs[i].length>0){
                        var _li = '<li><img onload="enableSaveBtn()" alt="" src="'+imgs[i]+'" style="max-width: 230px; width: auto; height: 230px;"></li>';
                        $(".imgs_list").append(_li);
                    }
                }
                var imglist = msg.imgurl.substring(0,msg.imgurl.lastIndexOf(","));
                ...
                var _li = '<li><img onload="enableSaveBtn()" alt="" src="'+msg.imgurl+'" style="max-width: 230px; width: auto; height: 230px;"></li>';
                $(".imgs_list").append(_li);
                if($("#fileName").val().length>0){
                    $("#fileName").val($("#fileName").val()+","+msg.imgurl);
                }else{
                    $("#fileName").val(msg.imgurl);
                }
            }
        }
    ...
    function enableSaveBtn() {
        imgLCount ++;
        if (imgLCount == imgSCount){ // 所有图片都加载完成
            imgModule.button({name: '保存',disabled: false});
            imgLCount = 0;
            imgSCount = 0;
        }
    }
