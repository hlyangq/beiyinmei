...
function submitReplyForm(customerNickname,commentId) {
    ...
    if(commentContent!=''){
        $.ajax({
            ...
            success:function(data) {
                $(".reply_form").find("p").first().before('<p>'+format(new Date().getTime(),"yyyy-MM-dd HH:mm:ss")+' <em class="text-info"> 商城 </em> 回复 <em class="text-info">' + customerNickname + '</em> ：'+commentContent+'<a style="margin-left:8px;" href="javascript:;" onclick="updateCommentRepDisplay(this)" display="1" url="updatecommentrep.htm?commentId='+commentId+'&replayId=' + data + '&isDisplay=1&CSRFToken=' + $("#CSRFToken").val()+'" role="button" class="btn btn-sm btn-default">点击显示</a></p>');
                $("#commentContent").val("");
            }
        });
    }
