...
function toStep3() {
   ...
   // checkInput("saveTypeForm","expandnicknames");
    ...
    if(!$("#saveTypeForm").valid()) return;
    if(!bool){
        showTipAlert("添加的属性名不能重复！");
        return;
    }
    if(!booll){
        showTipAlert("可选值不能重复!");
        return;
    }
    showSpecs("specs");
    $('.add_type2').hide();
    ...
    var html =
        '<tr id="expand_tr'+expandCount+'">'+
            '<td>'+
                ...
                '<input type="text" name="expandnames" class="w100" onblur="check($(this))">'+
            '</td>'+
...
function showUpdateType(typeId) {
    ...
    $.get("findTypeVoByTypeId.htm?CSRFToken="+$("#CSRFToken").val()+"&typeId=" + typeId, function (data) {
        ...
        for (var i = 0; i < data.expandParams.length; i++)
        {
            ...
            "<td>" + "<input type='hidden' name='expandparamId' value='" + data.expandParams[i].expandparamId + "'/>" + "<input type='hidden' name='expandparamDelflag' value='" + data.expandParams[i].expandparamDelflag + "'/>" + "<span class='text-danger'>*</span><input type='text' name='expandnames' value='" + data.expandParams[i].expandparamName + "' onblur='check($(this))'></td>" +
...
function addOneUpdateExpand() {
    ...
    var html =
        ...
        '<td>'+
        '<span class="text-danger">*</span>'+
        '<input type="hidden" name="expandparamId" value="-1"/><input type="hidden" class="expandparamDelflag" name="expandparamDelflag" value="0"/><input type="text" name="expandnames" class="w100" clazz="required specstr" onblur="check($(this))">'+
        '</td>'+
        '<td><input type="text" name="expandnicknames" class="w100"></td>'+
...
function check(obj){
    if($(obj).val() != ""){
        $(obj).parent().parent().siblings().find("input[name=expandnames]").each(function(){
            if($(this).val() != ""){
                if($(this).val() == $(obj).val()){
                    bool = false;
                    showTipAlert("添加的属性名不能重复！");
...
function checki(obj){
    if($(obj).val() != ""){
        $(obj).parent().find("input[name=tabinput]").each(function(){
            if($(obj).val() != ""){
                if($(obj).parent().find("input[value="+$(obj).val()+"]").length > 0){
                    booll = false;
                    showTipAlert("可选值不能重复!");
                }else{
                    booll = true;
