...
$.get("queryProductForCoupon.htm?CSRFToken="+$("#CSRFToken").val()+"&groupId=&pageNo="+pageNo+"&pageSize=10"+productNo+productName+brandId+goodsCateId,
    function (data)
    {
        ...
        for (var i = 0; i < list.length; i++) {
            ...
            productListHtml += "<td><input type='text' value='1' style='width: 50px; text-align: center' class='productNum' onblur='checkReg($(this))' onfocus='clearClass($(this))'></td>";
...
function submitSaveGroupProductForm() {
    ...
    for(var j = 0; j < checkboxs.length; j++) {
        if($(checkboxs[j]).is(':checked')==true) {
            var $productNum = $(checkboxs[j]).parent().parent().find("input[class=productNum]");
            if(!/^[1-9][0-9]*$/.test($productNum.val())){
                $productNum.addClass("error").parent().append("<br/><span style='color:#a94442'>请输入大于0的整数!</span>");
                return;
            }
            $productNum.attr("name","num");
            oneSelect = true;
        }
    }
    if(!oneSelect) {
...
function checkReg(obj){
    if(!/^[1-9][0-9]*$/.test($(obj).val())){
        $(obj).addClass("error").parent().append("<br/><span style='color:#a94442'>请输入大于0的整数!</span>");
    }
}