window.onload = function ()
{
    ...
    {
        var sturts = $("#depotSturts").val();
        if(sturts == "1"){
            var oldPrice = $("#update_goodsInfoPreferPrice").val();
            $(".update_productPrice").val(oldPrice);
        }
        if($("#updateProduct").valid()){
            if($(".checkExistsFlag").val() == "0"){
                ...
                $('#addSku1').on('shown.bs.modal',function(e) {
...
function checkUpdateSpec()
{
    ...
    for(var i = 0;i<productPrice.length;i++){
    	bValid = checkRegexp( $(productPrice[i]), /^(0|[1-9][0-9]{0,9})(\.[0-9]{1,2})?$/, "",null ) && bValid;
        if(!bValid){
            showTipAlert("请输入合法且大于0的货品价格！");
            return bValid;
        }
    }
    return bValid;
