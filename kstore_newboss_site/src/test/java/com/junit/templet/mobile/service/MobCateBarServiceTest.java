...
public class MobCateBarServiceTest extends UnitilsJUnit3 
{
     ...
     */
    @Test
    public void testselectStoreListQianggouImage() {
        PageBean pageBean = new PageBean();
        Map<String, Object> map = new HashMap<String, Object>();
        map.put("startRowNum", pageBean.getStartRowNum());
        map.put("endRowNum", pageBean.getEndRowNum());
        mobCateBarMapperMock.returns(1).selectStoreListQianggouImageCount();
        mobCateBarMapperMock.returns(new ArrayList<>()).selectStoreListQianggouImage(map);
        assertNotNull(mobCateBarService.selectStoreListQianggouImage(pageBean));
    }
    ...
    /**
     * 查询所有未删除、已启用的一级分类导航，返回List，前台调用测试
     */
    @Test
    public void testSelectOneMobCate() {
        mobCateBarMapperMock.returns(new ArrayList<>()).selectOneMobCate();
        assertNotNull(mobCateBarService.selectOneMobCate());
    }
     ...
     */
    @Test
    public void testQueryMobcateBarById() {
        mobCateBarMapperMock.returns(new ArrayList<>()).queryMobcateBarById(1L);
        assertNotNull(mobCateBarService.queryMobcateBarById(1L));
    }

    /**
     * 根据父分类导航ID查询显示的未删除的子分类导航分类
     */
    public void testQueryUsingMobCateBar() {
        mobCateBarMapperMock.returns(new ArrayList<>()).queryUsingMobCateBar(1L);
        mobCateBarService.queryUsingMobCateBar(1L);
    }
