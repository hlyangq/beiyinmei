...
public class MarketingServiceTest extends UnitilsJUnit3 {
    ...
    public void testMarketOrderList(){
        ...
        marketingMapperMock.returns(1).marketOrderListCount(paramMap);
        /** 设置开始行数 */
        paramMap.put("start", pageBean.getStartRowNum());
        /** 设置结束行数 */
        paramMap.put("number", pageBean.getEndRowNum());
        marketingMapperMock.returns(marketingList).marketOrderList(paramMap);
        assertNotNull(marketingService.marketOrderList(pageBean, marketingList.get(0), "null","null").getList().size());
    }
     ...
     * 分页查询促销列表
     */
    public void testQuerySimpleMarketingById(){
        marketingMapperMock.returns(marketing).marketingDetail(1L);
        assertEquals(marketing,marketingService.querySimpleMarketingById(1L));
