...
<head>
	...
	<title>积分商品详情页-${topmap.systembase.bsetName}</title>
	...
	.thumb_scroll_wp { width: 305px!important; margin: 0 auto;overflow: hidden;}
	</style>
</head>
<body>
...
<input type="hidden" id="limitnum" value="${gift.giftLimitBuy}">
   ...
   <div class="mask"></div><!--/mask-->
    <div class="dialog dia1">
        ...
        <div class="dia_cont">
            ...
            <div class="dia_ops mt20 tc">
                <a class="go_pay"  href="${basePath}/giftdetail/${gift.giftId}.html">确定</a>
            </div>
    ...
    if(li_n > 0) {
        $(".thumb_scroll_wp ul").width($(".thumb_scroll_wp li").length*61);
    ...
    $(".num_plus").click(function(){
        var count = parseFloat($(".product_buy_num").val());
        $(".product_buy_num").val(parseFloat(count+parseFloat(1)))
    });
    //减少积分兑换数量
