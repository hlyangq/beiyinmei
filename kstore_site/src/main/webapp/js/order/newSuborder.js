            ...
            $.each(data,function(key,value){
                ...
                if(data!=null&&data!=''){
                    ...
                    if(key=="couponList"){
                        ...
                        '<a class="open_link red" onclick="openLink(this);" href="javascript:;">[点此输入优惠券券码]</a>'+
                                '<div class="mt10" style="display:none;">'+
                                '<input type="text" class="text fl" id="useCoupon" style="width: 200px" maxlength="50"/>'+
                        '<input type="button"  class="common_btn" value="使用" onclick="checkCoupon()" style="width: 60px; height: 25px; display:inline-block; *display:inline; *zoom:1;"/>'+
...
function loadAllAddress(){
    ...
    $.ajax({type:"post",url:"../goods/queryCustAddressForSubOrder.html",async : false,success:function(data) {
        ...
        for (var i = 0; i < data.addresses.length; i++) {
            ...
            "><span limit='3' title='" + address.addressName + " " + address.province.provinceName + "'>" + strName + " " + address.province.provinceName + "</span><input type='hidden' class='district' value='"+address.district.districtId+"'/><b></b></div> " +
            "<div class='addr-detail'><span class='addr-name ml10' limit='6' title='" + address.addressName + "'>" + strName + "</span>" +
...
//	$('#amount').val('');
    ...
    //boss订单总价格
    var bosssum= accAdd($('#bosssumPrice').val(),$("#bossyfprice").val());
    var pointSet = $('#pointSet').val();          //积分兑换规则
    var amount = $('#amount').val();                //需要兑换的积分
