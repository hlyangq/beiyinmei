...
public class GiftOrderServiceImpl implements GiftOrderService {
    ...
    public int subOrder(GiftOrder giftOrder, Long giftStock) {
        ...
        CustomerPoint customerPoint = couponService.selectCustomerPointByCustomerId(giftOrder.getCustomerId());
        Long integral = customerPoint.getPointSum() - customerPointServiceMapper.getCustomerReducePoint(giftOrder.getCustomerId() + "");

        if (integral.longValue() < gift.getGiftIntegral().longValue()) {
            return 0;
        }
