...
public class ShareController {
    ...
    @ResponseBody
    public String saveShare(@Valid Share share, Comment comment, Long orderGoodsId, Long orderId, String imageNames, HttpServletRequest request) {
        if (LoginUtil.checkLoginStatus(request)) {
            // 当前登录会员id
            Long customerId = (Long) request.getSession().getAttribute(CustomerConstantStr.CUSTOMERID);
            // 订单不存在
            Order order = siteOrderService.queryOrderByCustomerIdAndOrderId(customerId, orderId);
            if (order == null) {
                return ERRORINFO;
            }
            // 订单商品不存在
            OrderGoods orderGoods = siteOrderService.queryOrderGoodsByOrderIdAndOrderGoodsId(orderId, orderGoodsId);
