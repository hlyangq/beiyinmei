...
public class AddressSiteController {
    ...
    public boolean ajaxAddCustomerAddressFromOrder(@Valid CustomerAddress address, BindingResult bindingResult, HttpServletRequest request) throws UnsupportedEncodingException {
        ...
        if (checkLoginStatus(request)) {
            ...
            //提交订单时添加收货地址，如果用户没有添加过地址，则此时添加的设为默认地址
            if(customerServiceInterface.queryAddressByCustomerId(customerId,pb).getRows() == 0){
    ...
    public CustomerAllInfo queryCustAddressByCustId(HttpServletRequest request) {
        ...
        if (customerAllInfo != null) {
            ...
            customerAllInfo.setLoginKey(null);

            customerAllInfo.setInfoCardid(null);

            customerAllInfo.setPointLevelId(null);

            customerAllInfo.setBalanceSum(null);

            customerAllInfo.setCustomerUsername(null);

            customerAllInfo.setSaltVal(null);
            // 非空验证 用户名
            if (null != customerAllInfo.getCustomerUsername()) {
    ...
    public boolean isCusAddsLegal(HttpServletRequest request) {
        if (checkLoginStatus(request)) {
            ...
            Long customerId = getLoginUserId(request);
            // 判断用户是否可以新增地址
            return this.customerServiceInterface.isCustomerAddressNumLegal(customerId);
        }else {
            return false;
        }
    }
